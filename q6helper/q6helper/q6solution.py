...
def append_ordered(ll,v):
    if ll == None or v < ll.value:
        return LN(v,ll)

    t = ll
    while t.next is not None and v >= t.next.value:
        t = t.next 
    t.next = LN(v,t.next)
    return ll
#     while True:
#         if t.next is None or v < t.next.value:
#             t.next = LN(v,t.next)
#             return ll
#         t = t.next 
...
def max_depth(t,value):
    def max_depth_h(t,value,depth):
        if t == None:
            return -1
        else:
            left_max  = max_depth_h(t.left,  value, depth+1)
            right_max = max_depth_h(t.right, value, depth+1)
            if t.value == value:
                return max(depth, left_max, right_max)
            else:
                return max(left_max, right_max)
        
    return max_depth_h(t,value,0)
...
class StringVar_WithHistory(StringVar):
    def __init__(self):
        ...
        self.history = []                        # value at [-1] is most recent value
