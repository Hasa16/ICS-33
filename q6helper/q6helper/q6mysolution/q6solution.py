...
def append_ordered(ll,v):
    ...
    if ll is None:
        ll = ln1
    elif(ll.value >= ln1.value):
        ln1.next = ll
        ll = ln1
    else:
        ll2 = ll
        while (ll2.next is not None and ll2.next.value < ln1.value):
            ll2 = ll2.next
        ln1.next = ll2.next
        ll2.next = ln1
    return ll
...
def append_ordered_r(ll,v):
    ...
    if ll is None:
        ...
        while (ll2.next is not None and ll2.next.value < ln1.value):
            ll2 = ll2.next
        ln1.next = append_ordered_r(ln1, ll2.value)
        ln1.next = ll2.next
        ll2.next = ln1
    return ll
...
def max_depth(t,value):
    if t == None:
        ...
        def max_depth2(t1, value, depth=0):
            leftMax = -1
            rightMax = -1
            mx = -1
            
            if t1.left:
                leftMax = max_depth2(t1.left, value, depth+1)
            
            if t1.right:
                rightMax = max_depth2(t1.right, value, depth+1)
            
            if t1.value == value:
                mx = depth
                    
            return max(mx, leftMax, rightMax)
        return max_depth2(t, value)
...
class StringVar_WithHistory(StringVar):
    def __init__(self, **kwargs):
        ...
        self.history = []
        if 'value' in kwargs:
            self.history.append(kwargs['value'])
